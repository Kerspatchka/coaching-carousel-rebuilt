...
- NFL departures should target coordinators more frequently than HCs. HC-to-NFL moves are rare headline events; exact destinations are not invented. Any scoring input must come from reliable current-save evidence or an explicit labeled configuration input.
...
M3 is now underway. A pure market initializer converts the normalized snapshot into exactly one HC, OC, and DC seat per Team, enforces unique and role-coherent incumbents, derives a reproducible default seed from season and source fingerprint, records user Coach/Team context, and preserves native staged openings only as non-authoritative evidence. The first pure Part 1 evaluator now consumes Team offensive/defensive ranks, unit ratings, Team records and prestige, expected contract points, Coach three-year earned contract points, job security, tenure, grace, prestige, and résumé evidence. It applies the approved role-specific weights and calibrated HC 32 / OC 24 / DC 32 Fire cutoffs; missing prior-year contract evidence is neutral rather than poor; an unconditional Fire requires at least two independent primary signals; and grace-protected firing requires Catastrophic Failure. The supported fixture initializes and evaluates all 429 seats as 33 Fire (12 HC/11 OC/10 DC), 119 Vulnerable, and 277 Secure, with three catastrophic results. Exact fixture totals are regression-locked while downstream turnover calibration remains open. The renderer now advances from `ENGINE READY` into a real-data `STAFF EVALUATED` review with a consolidated title/save/status header, a vertically efficient strip of complete clickable Fire/Vulnerable/Secure/Grace tabs, combinable All/HC/OC/DC and Conference filters, fully expanding portrait-and-flat-helmet Coach cards, role-specific unit and contract evidence, enlarged readable type, a list-only scrollbar, fixed page-level actions, and a dedicated high-resolution layout tier. Every classification or secondary-filter change uses a reduced-motion-aware staggered list reveal. The separate Louisville market remains explicitly labeled `CAROUSEL PREVIEW`. Departure events, buyout affordability, contract decisions, candidate scoring, shortlists, pricing, and offer-round resolution remain next.
...
1. Complete Part 1 above the implemented evaluation layer: seeded retirement/NFL/unexpected departures, user protection decisions, early-termination buyout quotes and affordability, `Cleans House`, contract decisions, and initial vacancies. Do not enable production buyout settlement until its native financial write path is validated.
2. Replace the remaining fixture preview with the complete deterministic Part 2 engine, consuming real evaluated state and finalizing eligibility, shortlists, interest, valuation, pricing, affordability reservations, generic-Coach fallbacks, and offer-round resolution through multi-seed tests.
3. Connect the proven compiler and automated copy/write/reopen verification behind Part 3 final authorization, then complete reports, handoff, and clean-machine packaging tests.
4. Optimize and, where safe, physically deduplicate the production asset pack while preserving Coach, school, and conference identity mappings; finalize licensing/public-distribution policy before a public release.
5. Perform additional source analysis or human-operated game experiments only when a specific unresolved implementation decision cannot be answered from existing evidence.
