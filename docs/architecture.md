...
The implemented transition from `ready` constructs a pure `MarketBaseline`: 3 seats per normalized Team, unique role-coherent incumbents, deterministic seeded ordering inputs, user contexts, and native staged outcomes retained only as evidence. A normal UI initialization supplies a fresh seed containing Unix time in milliseconds plus season/save identity; tests, calibration, and reruns supply an explicit seed. A second pure transition produces `PartOneEvaluation` from that baseline and the same normalized snapshot. It retains every weighted component, reason detail, failure signal, grace result, catastrophic-failure result, and user-confirmation requirement.

A third pure transition, `planPerformanceActions`, converts Fire classifications into a provisional in-memory action graph. A CPU HC Fire result creates one `Fired` action plus two `StaffNotRetained` releases and three vacancies; an independently fired coordinator creates only its own action and vacancy. Protected user HC firings and coordinator recommendations at a user-HC school become explicit pending decisions, and a retain choice becomes an auditable override. Every under-contract release is marked `buyoutRequired`, and the plan remains non-finalizable until the later buyout-pricing and affordability command succeeds. The planner checks source-fingerprint identity, complete Team staffs, unique released Coaches, and one-to-one action/vacancy ownership.

Seeded probabilistic systems use keyed rolls rather than consuming a mutable global RNG stream. A roll is derived from the run seed plus a stable event/entity key, so adding or reordering another Coach does not silently reroll existing candidates. Every roll retains its exact probability, value, and result. Seasonal allowances use deterministic weighted sampling without replacement and retain weight, roll, priority, and selection evidence. The save adapter normalizes the unique finalized week-20 `SeasonGame` championship record, cross-checks `GameStatus` against its score, resolves both Team references, and fails preflight if the result is missing, ambiguous, or incoherent. `planPartOneDepartures` first resolves NFL departures, then passes that normalized winner Team ID into retirement while excluding NFL-selected Coaches, then excludes all previously affected Coaches and Teams from Unexpected Scenario targeting before combining the three ledgers and rejecting duplicate departures or vacancies. Retirement is 0% through age 60 and applies an evidenced age curve, season, contract, career-length, and authoritative `Retiring on Top` modifier from age 61 onward. Unexpected Scenarios first draw a zero/one/two seasonal allowance, select at most one Coach per unaffected Team through role weights, then choose an approved fictional category and consequence. `Looking for a Change` is recorded as a voluntary resignation: the Coach vacates the current seat but is added to the available-Coach pool and incurs no school-initiated buyout. A scenario that would remove a protected user HC becomes a pending accept/nullify decision; nullification consumes the selected scenario without rerolling. Performance Fire classifications remain deterministic and receive the combined departed-Coach and HC-succession sets.
