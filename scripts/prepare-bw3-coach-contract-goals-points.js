/* Prepare E1E: one-off Air Force role-budget reduction plus coach-contract-goals credit. */
'use strict';
...
function parseArgs(argv) {
  const options = {
    ...
    outputDirectory: path.join(__dirname, '..', 'assets', 'experiments', 'bw3-coach-contract-goals-points')
  };
...
function pointState(state) {
  ...
  return {
    ...
    coachContractGoals: team.CoachContractGoalsProgramPoints,
...
function applyTreatment(state) {
  ...
  team.CoachContractGoalsProgramPoints += AMOUNT;
}
...
async function main() {
  ...
  assert(before.headCoachBudget === 330 && before.coachContractGoals === 0, 'Unexpected Air Force coach-budget baseline.');
  assert(before.rollover === 0 && before.programPointBudget === 1250 && before.staffPool === 540, 'Unexpected Air Force budget baseline.');
  const sourceSnapshot = focusedSnapshot(baseline.tables);
  const output = path.join(options.outputDirectory, 'DYNASTY-CCRY1BW3-E1E-GOALS25');
  ...
  assert(after.coachContractGoals === before.coachContractGoals + AMOUNT, 'CoachContractGoalsProgramPoints treatment did not persist.');
  assert(after.rollover === before.rollover, 'RolloverProgramPoints changed unexpectedly.');
  assert(after.programPointBudget === before.programPointBudget, 'ProgramPointBudget changed unexpectedly.');
  ...
  const expected = new Set(['HeadCoachProgramPointBudget', 'StaffProgramPointsSpent', 'RemainingProgramPoints', 'CoachContractGoalsProgramPoints']);
