/* Prepare E1C: one-off Air Force role-budget reduction plus rollover credit. */
'use strict';
...
function parseArgs(argv) {
  const options = {
    ...
    outputDirectory: path.join(__dirname, '..', 'assets', 'experiments', 'bw3-program-points-rollover')
  };
...
function applyTreatment(state) {
  ...
  team.RolloverProgramPoints += AMOUNT;
}
...
async function main() {
  ...
  assert(before.headCoachBudget === 330 && before.rollover === 0, 'Unexpected Air Force authority-field baseline.');
  assert(before.programPointBudget === 1250 && before.staffPool === 540, 'Unexpected Air Force budget baseline.');
  const sourceSnapshot = focusedSnapshot(baseline.tables);
  const output = path.join(options.outputDirectory, 'DYNASTY-CCRY1BW3-E1C-ROLLOVER25');
  ...
  assert(after.rollover === before.rollover + AMOUNT, 'RolloverProgramPoints treatment did not persist.');
  ...
  const expected = new Set(['HeadCoachProgramPointBudget', 'StaffProgramPointsSpent', 'RemainingProgramPoints', 'RolloverProgramPoints']);
